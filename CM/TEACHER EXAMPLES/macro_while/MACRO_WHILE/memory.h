 ...
 * le programme si la demande echoue.
 */
...
#define MY_MALLOC(pt, type, nbElt) \
    do { \
        ...
        if ((pt) == NULL) { \
            fprintf(stderr, \
                    "Ereur allocation dans le fichier %s a la ligne %d " \
                    "dans la fonction %s :\n", \
...
#define MY_REALLOC(pt, ptInit, type, nbElt) \
    do { \
        ...
        if ((pt) == NULL) { \
            fprintf(stderr, \
                    "Ereur reallocation dans le fichier %s a la ligne %d " \
                    "dans la fonction %s :\n", \
                    __FILE__, __LINE__, __func__; \
            fprintf(stderr, \
                    "    on voulait allouer %s elements de type %s\n", \
 ...
 * ce ne soit plus/pas nécessaire. Le pointeur libere est mis a NULL
 */
#define MY_FREE(pt) \
    do { \
        if ((pt) != NULL) { \
